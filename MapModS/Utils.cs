...
namespace MapModS
{
    public static class Utils
    {
        ...
        private static string RemoveBossSuffix(string scene)
        {
        ...
        {
            if (MapData.RoomLookup.TryGetValue(CurrentScene(), out MapPositionDef mpd))
            {
                return mpd.MapZone;
            }
            // This seems to be the one mapped room that doesn't have its MapZone set
            if (CurrentScene() == "Ruins_Elevator")
            {
                return MapZone.CITY;
            }
            return default;
