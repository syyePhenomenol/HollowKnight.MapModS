...
namespace MapModS.Map
{
    public static class PinsVanilla
    {
                ...
                try
                {
                    switch (child.name)
                    {
                        ...
                        case "pin_sly (1)":
                            ...
                            //_Groups[Pool.Vendor].Add(child.gameObject);
                            _PinsToDisable.Add(child.gameObject);
                            break;
        ...
        public static void RefreshGroups()
        {
            ...
            _BenchPins.RemoveAll(item => item == null);
            _PinsToDisable.RemoveAll(item => item == null);

            foreach (GameObject pinObject in _BenchPins)
            {
                pinObject.SetActive(false);
            }

            foreach (GameObject pinObject in _PinsToDisable)
            {
                pinObject.SetActive(false);
            }
